...
test("Multiple Tabs",async({browser}) => {
    ...
    let [page2]=await Promise.all([
        ...
        page.locator('(//img[@class="MZeksS"])[3]').click()
    ])
...
test("Task 2",async({browser}) => {
    ...
    await page2.pause();        // opens playwright inspector (debug mode)
})
...
// new Window
// Tab vs Window doesn't matter to Playwright
// Both are handled using "popup"
...
test.only("Task 4",async({browser})=>{
    ...
    await download.saveAs('newFile.txt')
